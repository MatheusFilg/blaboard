...
export function EditLabelModal({ isOpen, onClose, onSubmit, label }: EditLabelModalProps) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await onSubmit(label.id, { text: labelText, color: color.toString("hex") });
      onClose()
    } finally {
      setIsEditing(false);
    }
  ...
  const hasChanged = labelText !== label.text || color.toString("hex") !== label.color;
